// modules/stacks/components/TeamTaskList.tsx
import React, { useState, useEffect } from 'react';
import teamTasksService from '../services/team_tasks_service_mock';
import { Task, TaskStatus, TaskPriority } from '../types/teamStasks';
...
const TeamTaskList: React.FC<TeamTaskListProps> = ({
  ...
  const renderAssignees = (assignees: any[]) => {
    if (assignees.length === 0) return 'Chưa gán';
    if (assignees.length === 1) return assignees[0].name;
    return `${assignees[0].name} và ${assignees.length - 1} người khác`;
  };
